...
def kontrol_keyboard(keluar_callback):
    ...
    while True:
        if keyboard.is_pressed('space'):
            ...
            paused = False
            dihentikan = True  # ✅ Set flag supaya lirik stop juga
...
def sing_song(lagu_data, keluar_callback):
    global dihentikan
    dihentikan = False  # ✅ Reset setiap mulai lagu baru
    ...
    for i in range(len(lyrics)):
        if dihentikan:  # ✅ Stop loop jika lagu dihentikan
            break
    ...
    if not dihentikan:
        animate_text("\n♪ Lagunya sudah selesai. Semoga Anda menikmatinya. ♪", 0.05)
